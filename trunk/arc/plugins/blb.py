...
class BlbPlugin(ProtocolPlugin):
    ...
    def commandNBlb(self, parts, fromloc, overriderank):
        ...
        if len(parts) < 8 and len(parts) != 2:
            ...
            if realLimit >= 32*32: # To test it out first, will try a bigger one later - tyteen
    ...
    def commandBlb(self, parts, fromloc, overriderank):
        ...
        if len(parts) < 8 and len(parts) != 2:
            ...
            def do_step():
                # Do 10 blocks
                try:
                    for x in range(100): # 10 blocks at a time, 10 blocks per tenths of a second, 100 blocks a second
                        block_iter.next() # lol fuk u i set it to 100 blocks per tenth. modern internet is fast.
